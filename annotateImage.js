...
url = 'https://vision.googleapis.com/v1/images:annotate?key=AIzaSyCe31QbxiHWLedN6egRGDvckIQCMSj_dTU';
...
function annotateImage() {
	...
	function APIcallback(err, APIresponse, body) {
		...
		console.log("landmark: ",APIresponseJSON.landmarkAnnotations[0].description);

		var tags = APIresponseJSON.labelAnnotations;
		var tagsStr = '';
     	for (let i = 0; i < 6; i++) {
        	tagsStr += tags[i].description + ",";
     	}
		 ...
		 //UPDATE photoTags
		 //SET landmark = APIresponseJSON.landmarkAnnotations[0].description, tags = tagsStr,
		 //WHERE landmark !== undefined; //NOT SURE IF THIS WORKS
